...
const menuItems = [
  { 
    ...
    icon: BarChart3, 
    badge: null,
    roles: ['admin', 'manager', 'receptionist', 'housekeeping', 'kitchen']
  },
  { 
    id: 'front-desk', 
    label: 'Front Desk', 
...
export function HotelSidebar({
  ...
  const handleLogout = async () => {
    try {
      // Remove the signOut import since it's not defined
  ...
  const filteredMenuItems = menuItems.filter(item => 
    item.roles.includes(safeUser.role)
  )
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 p-2 scrollbar-hide">
        ...
        <div className="mt-auto">
          ...
          <Button
            variant="ghost"
            className={`
              w-full justify-start gap-3 px-3 text-gray-700 hover:bg-gray-50
          ...
          <Button
            variant="ghost"
            className={`
              w-full justify-start gap-3 px-3 text-red-600 hover:bg-red-50
              ${isCollapsed && !isMobile ? 'justify-center px-2' : ''}
            `}
